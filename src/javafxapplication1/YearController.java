...
public class YearController implements Initializable {
    ...
    private void listvclicked(MouseEvent event) {
        String abc = (String) yearlist.getSelectionModel().getSelectedItem();
//    System.out.println("clicked on " + yearlist.getSelectionModel().getSelectedItem());
        yeartable.getColumns().clear();
        Connection c;
        data = FXCollections.observableArrayList();
        try {
            c = (Connection) db.Db();
            String SQL = "SELECT movies.title,movies.year,movies.genre,movies.price from movies where year = '" + abc + "' ";
            //ResultSet
            ...
            for (int i = 0; i < rs.getMetaData().getColumnCount(); i++) {
                ...
                yeartable.getColumns().addAll(col);
            }

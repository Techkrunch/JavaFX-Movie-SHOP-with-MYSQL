...
public class AllMoviesController implements Initializable {
    ...
    private void loadallmovies() {
        allmovies.getColumns().clear();
        Connection c;
        data = FXCollections.observableArrayList();
        try {
            c = (Connection) db.Db();
            String SQL = "SELECT movies.title,movies.year,movies.genre,movies.price FROM `movies`";
            //ResultSet
            ...
            for (int i = 0; i < rs.getMetaData().getColumnCount(); i++) {
                ...
                allmovies.getColumns().addAll(col);
            }
